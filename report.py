...
class ExcelEditor:
    ...
    def __init__(self, root):
        ...
        # Исправлено создание виджета Treeview
        self.tree = ttk.Treeview(self.tree_frame, columns=tuple(f"Col{i}" for i in range(1, 16)), show="headings",
                                 selectmode="extended")
        for i in range(1, 16):
            col_id = f"Col{i}"
            col_text = f"Колонка {i}" if i <= len(self.COLUMN_NAMES) else f"Колонка {i}"
